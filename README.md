...
`xgbpid` classifies particles passing through a plastic scintillator (**electron**, **pion**, or **kaon**) in under 1 millisecond. It was built for the Beamline for Schools (BL4S) competition at CERN by Team TEAMNAME consisting of 9 students from Robert College, Türkiye.
...
The **teacher** is a tiered array of existing CERN detectors that provides ground-truth particle labels:
...
- **Lead-glass calorimeter**: Confirms electrons by detecting electromagnetic showers
...
In simulation mode (`use_simulation: true` in the config), a `MockDAQ` generates simulated pulses with realistic noise and timing jitter for each species. This allows for development and trianing without being at CERN.

---

### Step 2 - Feature Extraction (`xgbpid/core/processor.py`)

Raw waveforms aren't fed into the model directly. Instead, six physics-derived features are computed from each pulse:
...
Pile-up detection runs first. If more than one distinct pulse is found above threshold, the event is discarded. The check requires at least 5 consecutive samples above threshold to count as a second pulse, preventing prevents kaon long-tails from being falsely rejected.

---

### Step 3 - Particle Classification (`xgbpid/core/inference.py`)

The six features are handed to an XGBoost multi-class classifier. The model outputs a probability for each class and returns the one with the highest confidence.
...
The initial model is trained using `MockDAQ` with balanced class sizes (equal numbers of pions, electrons, and kaons). Balanced training means the model learns pulse-shape differences, not beam fractions. At runtime the real beam (~85% pions, ~8% electrons, ~1.5% kaons) is naturally reflected in the event stream.
...
During a live run, the **background retrainer** wakes up every 60 seconds (configurable), loads all accumulated Parquet run files, filters to events with confirmed teacher labels, and retrains the XGBoost model. The new model is atomically written to `models/xgbpid.json`, where the classifier picks it up on the next hot-reload check.

---

### Step 5 - Telemetry & Dashboard (`dashboard.py`)

Every 50 events (configurable), the main loop atomically overwrites `data/live_telemetry.parquet` using a write-to-temp-then-rename pattern. The rename is atomic on POSIX systems, so the dashboard always reads a complete file, no locking, is needed.
...
- **Rolling particle composition**: Bar and pie charts of predicted labels
...
- **Kaon event log**:A dedicated section for the rare kaon candidates
...
Defaults reflect true physics: electrons have fast, narrow pulses, pions are slower, and kaons are the slowest with the longest tail.
...
If either variable is missing, the relay silently disables itself and the rest of the system runs normally. The `.env` file is never committed to source control.
...
At 5–10 GeV/c, all three species are ultra-relativistic, making traditional Cherenkov-based separation by velocity difficult. This is exactly why pulse-shape discrimination is interesting: the differences in how each particle deposits energy in the scintillator (rise time, tail length, decay shape) provides species information even when velocity-based methods break down.

Electrons deposit energy electromagnetically (fast, narrow pulses). Pions and kaons deposit energy hadronically (slower, longer tails). Kaons are heavier than pions, produce a broader hadronic shower, and have a noticeably slower rise and longer tail. These subtle waveform differences are what the XGBoost model learns to distinguish.
